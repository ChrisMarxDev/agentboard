#!/usr/bin/env bash
# Smoke test for the files-first store at /api/<key> +
...
# --- Singleton ---
assert_status "PUT singleton fresh"      PUT "/api/smoke.k" 200 -H 'Content-Type: application/json' -d '{"value":42}'
assert_jq     "GET singleton value"      "/api/smoke.k" ".value" "42"
assert_jq     "shape == singleton"       "/api/smoke.k" "._meta.shape" "singleton"

VERSION=$(authed_curl "$URL/api/smoke.k" | jq -r "._meta.version")

assert_status "PUT with matching ver"    PUT "/api/smoke.k" 200 \
  -H 'Content-Type: application/json' -d "{\"_meta\":{\"version\":\"$VERSION\"},\"value\":100}"

assert_status "PUT with stale ver -> 412" PUT "/api/smoke.k" 412 \
  -H 'Content-Type: application/json' -d '{"_meta":{"version":"1999-01-01T00:00:00Z"},"value":999}'

# --- Merge (never conflicts) ---
assert_status "PATCH merge create"       PATCH "/api/smoke.cfg" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"a":1,"b":2}}'
assert_status "PATCH merge update"       PATCH "/api/smoke.cfg" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"a":99,"c":3}}'
assert_jq     "deep merge preserved b"    "/api/smoke.cfg" ".value.b" "2"
assert_jq     "deep merge updated a"      "/api/smoke.cfg" ".value.a" "99"
assert_jq     "deep merge added c"        "/api/smoke.cfg" ".value.c" "3"

# No atomic field-level increment / CAS — agents read-modify-write
# the whole doc under file-level _meta.version CAS.

# --- Collection ---
assert_status "Upsert task-1"            PUT "/api/smoke.kanban/task-1" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"title":"a","col":"todo"}}'
assert_status "Upsert task-2"            PUT "/api/smoke.kanban/task-2" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"title":"b","col":"todo"}}'
assert_jq     "list 2 items"             "/api/smoke.kanban" ".items | length" "2"
assert_jq     "merge_by_id preserves"     "/api/smoke.kanban/task-1" ".value.title" "a"
assert_status "MERGE_BY_ID col field"    PATCH "/api/smoke.kanban/task-1" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"col":"in-progress"}}'
assert_jq     "title preserved"           "/api/smoke.kanban/task-1" ".value.title" "a"
assert_jq     "col updated"               "/api/smoke.kanban/task-1" ".value.col" "in-progress"

# --- Stream ---
assert_status "Stream append single"     POST "/api/smoke.events:append" 200 \
  -H 'Content-Type: application/json' -d '{"value":{"e":"signup"}}'
assert_status "Stream append batch"      POST "/api/smoke.events:append" 200 \
  -H 'Content-Type: application/json' -d '{"items":[{"e":"login"},{"e":"view"},{"e":"logout"}]}'
assert_jq     "stream tail 4 lines"      "/api/smoke.events?limit=10" ".lines | length" "4"
assert_jq     "shape stream"             "/api/smoke.events" "._meta.shape" "stream"

# --- Wrong shape ---
assert_status "APPEND on singleton -> 409" POST "/api/smoke.k:append" 409 \
...
# --- History + activity ---
assert_jq     "history has entries"       "/api/smoke.k/history" ".entries | length > 0" "true"
...
# --- Delete ---
assert_status "Delete singleton"         DELETE "/api/smoke.k" 204
assert_status "GET after delete -> 404"  GET    "/api/smoke.k" 404
assert_status "Delete by id"             DELETE "/api/smoke.kanban/task-1" 204
assert_status "Delete collection needs confirm" DELETE "/api/smoke.kanban" 400
