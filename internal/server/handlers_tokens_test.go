...
// Token endpoints live at /_api/users/{username}/tokens/* with the
...
func TestTokens_AdminCanMintForAny(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/dana/tokens", `{"label":"ci"}`, adminToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 201 {
...
func TestTokens_MemberCanMintForSelf(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/dana/tokens", `{"label":"laptop"}`, daneToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 201 {
...
func TestTokens_MemberCannotMintForOther(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/eve/tokens", `{"label":"laptop"}`, daneToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 403 {
...
func TestTokens_MemberCannotMintForBot(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/helper/tokens", `{"label":"ci"}`, daneToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 403 {
...
func TestTokens_AdminMintsForBot(t *testing.T) {
	...
	_ = seedUserKind(t, srv, "helper", auth.KindBot)
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/helper/tokens", `{"label":"ci"}`, adminToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 201 {
...
func TestTokens_BotMintsForSelf(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/helper/tokens", `{"label":"second"}`, botToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 201 {
...
func TestTokens_SelfRevokeInUseRejected(t *testing.T) {
	...
	tokenID := tokens[0].ID
	r, err := http.DefaultClient.Do(authReq(t, "POST",
		ts.URL+"/_api/users/dana/tokens/"+tokenID+"/revoke", "", daneToken))
	if err != nil {
		t.Fatal(err)
	}
	if r.StatusCode != 400 {
...
func TestTokens_ListForSelf(t *testing.T) {
	...
	r, err := http.DefaultClient.Do(authReq(t, "GET",
		ts.URL+"/_api/users/dana/tokens", "", daneToken))
	if err != nil {
		t.Fatal(err)
	}
	var resp struct {
...
func TestMe_ReturnsSignedInUser(t *testing.T) {
	...
	// token, so a bare Get hits /_api/me authenticated.
	r, err := http.Get(ts.URL + "/_api/me")
	if err != nil {
		t.Fatal(err)
	}
	defer r.Body.Close()
	if r.StatusCode != 200 {
		t.Fatalf("/_api/me = %d", r.StatusCode)
	}
	var me struct {
	...
	if me.Username == "" {
		t.Errorf("/_api/me returned empty user: %+v", me)
	}
}
